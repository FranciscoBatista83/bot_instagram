...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.navigation_frame_label = ctk.CTkLabel(self.navigation_frame, text="Bot Instagram",
        ...
        self.config_button = ctk.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="Credenciais & Config",
                                            fg_color="transparent", text_color=("gray10", "gray90"),
                                            anchor="w", command=self.config_button_event)
        self.config_button.grid(row=2, column=0, sticky="ew")

        self.actions_button = ctk.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="Ações do Bot",
                                            fg_color="transparent", text_color=("gray10", "gray90"),
                                            anchor="w", command=self.actions_button_event)
        self.actions_button.grid(row=3, column=0, sticky="ew")

        self.logs_button = ctk.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="Logs & Monitoramento",
                                            fg_color="transparent", text_color=("gray10", "gray90"),
                                            anchor="w", command=self.logs_button_event)
        self.logs_button.grid(row=4, column=0, sticky="ew")

        self.data_button = ctk.CTkButton(self.navigation_frame, corner_radius=0, height=40, border_spacing=10, text="Dados",
                                            fg_color="transparent", text_color=("gray10", "gray90"),
        ...
        self.appearance_mode_optionemenu = ctk.CTkOptionMenu(self.navigation_frame, values=["System", "Dark", "Light"],
                                                               command=self.change_appearance_mode_event)
        self.appearance_mode_optionemenu.grid(row=9, column=0, padx=20, pady=(10, 20), sticky="ew")

        # Criar frames para cada aba
        self.dashboard_frame = DashboardTab(self, corner_radius=0, fg_color="transparent")
        self.config_frame = ConfigTab(self, self.instagram_user, self.instagram_password, corner_radius=0, fg_color="transparent")
        self.logs_frame = LogsTab(self, corner_radius=0, fg_color="transparent") # LogsTab deve ser criada antes
        self.actions_frame = ActionsTab(self, self.logs_frame, self.instagram_user, self.instagram_password, corner_radius=0, fg_color="transparent") # Passar logs_frame e credenciais
    ...
    def select_frame_by_name(self, name):
        ...
        self.actions_button.configure(fg_color=("gray75", "gray25") if name == "actions" else "transparent")
