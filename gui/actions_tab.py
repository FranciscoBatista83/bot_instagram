...
class ActionsTab(ctk.CTkFrame):
    def __init__(self, master, logs_tab_instance, instagram_user_var, instagram_password_var, **kwargs):
        ...
        self.process_thread = None # Adicionar para rastrear a thread do processo
        ...
        self.status_label.grid(row=7, column=0, padx=20, pady=(10, 20), sticky="ew")
    ...
    def run_bot(self, script_name):
        ...
        thread = threading.Thread(target=self._execute_bot_script, args=(script_name, username, password))
        thread.start()

    def _execute_bot_script(self, script_name, username, password):
        try:
            script_path = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'bots')), script_name)
            
            # Passar credenciais como argumentos de linha de comando
            command = [sys.executable, script_path, "--user", username, "--password", password]
